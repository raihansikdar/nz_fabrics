              ...
              child: Column(
                children: [
                  WaterChartContainerWidget(
                    ...
                    title: "Total",
                    energyText: "${controller.lineChartModel.individualTotals?.values?['Total']?.toStringAsFixed(0) ?? '0.00'} m³",
                    costText: "${controller.lineChartModel.individualTotalCost?.values?['Total']?.toStringAsFixed(0) ?? '0.00'} ৳",
                  ),
                  ...
                  WaterChartContainerWidget(
                    ...
                    title: "Sub Mersible",
                    energyText: "${controller.lineChartModel.individualTotals?.values?['Sub_Mersible']?.toStringAsFixed(0) ?? '0.00'} m³ (${controller.lineChartModel.percentages?.values?['Sub_Mersible']?.toStringAsFixed(2) ?? '0.00'}%)",
                    costText: "${controller.lineChartModel.individualTotalCost?.values?['Sub_Mersible']?.toStringAsFixed(0) ?? '0.00'} ৳",
                  ),
                  SizedBox(height: size.height * k8TextSize),
                  WaterChartContainerWidget(
                    size: size,
                    ...
                    title: "WTP",
                    energyText: "${controller.lineChartModel.individualTotals?.values?['WTP']?.toStringAsFixed(0) ?? '0.00'} m³ (${controller.lineChartModel.percentages?.values?['WTP']?.toStringAsFixed(2) ?? '0.00'}%)",
                    costText: "${controller.lineChartModel.individualTotalCost?.values?['WTP']?.toStringAsFixed(0) ?? '0.00'} ৳",
                  ),
                ],
              ),
            );
