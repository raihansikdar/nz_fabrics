...
class MonthlyWaterChartWidget extends StatelessWidget {
  const MonthlyWaterChartWidget({
    super.key,
    required this.size,
  });
              ...
              child: Column(
                children: [
                  WaterChartContainerWidget(
                    ...
                    title: "Total",
                    energyText: "${controller.monthlyBarchartModel.individualTotals?.values?['Total']?.toStringAsFixed(2) ?? '0.00'} m³",
                    costText: "${controller.monthlyBarchartModel.individualTotalCost?.values?['Total']?.toStringAsFixed(2) ?? '0.00'} ৳",
                  ),
                  ...
                  WaterChartContainerWidget(
                    ...
                    title: "Sub Mersible",
                    energyText: "${controller.monthlyBarchartModel.individualTotals?.values?['Sub_Mersible']?.toStringAsFixed(2) ?? '0.00'} m³ (${controller.monthlyBarchartModel.percentages?.values?['Sub_Mersible']?.toStringAsFixed(2) ?? '0.00'}%)",
                    costText: "${controller.monthlyBarchartModel.individualTotalCost?.values?['Sub_Mersible']?.toStringAsFixed(2) ?? '0.00'} ৳",
                  ),
                  SizedBox(height: size.height * k8TextSize),
                  WaterChartContainerWidget(
                    size: size,
                    ...
                    title: "WTP",
                    energyText: "${controller.monthlyBarchartModel.individualTotals?.values?['WTP']?.toStringAsFixed(2) ?? '0.00'} m³ (${controller.monthlyBarchartModel.percentages?.values?['WTP']?.toStringAsFixed(2) ?? '0.00'}%)",
                    costText: "${controller.monthlyBarchartModel.individualTotalCost?.values?['WTP']?.toStringAsFixed(2) ?? '0.00'} ৳",
                  ),
                ],
              ),
            );
