...
class _MonthlyDetailsBarChartWidgetState extends State<MonthlyDetailsBarChartWidget> with AutomaticKeepAliveClientMixin {
  ...
  // Optimized data preparation with full month data
  dynamic _prepareChartData() {
    try {
      final now = DateTime.now();
      ...
      // Generate full month data (1st to last day)
      ...
      if (widget.screenName == 'waterScreen') {
        // Handle water screen data
        final apiData = widget.waterMonthlyDataModel ?? [];
        final Map<DateTime, WaterChartData> dataMap = {};

        // Map API data
        for (var data in apiData) {
          try {
            final date = DateTime.parse(data.date ?? '');
            dataMap[DateTime(date.year, date.month, date.day)] = WaterChartData(
              date: date,
              instantFlow: data.instantFlow ?? 0.0,
              instantCost: data.cost ?? 0.0,
            );
        ...
        // Generate chart data for all days
        ...
        // Handle natural gas screen data
        final apiData = widget.naturalGasMonthlyDataModel ?? [];
        final Map<DateTime, NaturalGasChartData> dataMap = {};

        // Map API data
        for (var data in apiData) {
          try {
            final date = DateTime.parse(data.date ?? '');
            dataMap[DateTime(date.year, date.month, date.day)] = NaturalGasChartData(
              date: date,
              volume: data.volume ?? 0.0,
              cost: data.cost ?? 0.0,
            );
        ...
        // Handle power/energy screen data
        final apiData = widget.monthlyDataModelList ?? [];
        ...
        for (var data in apiData) {
          try {
            final date = DateTime.parse(data.date ?? '');
            dataMap[DateTime(date.year, date.month, date.day)] = ChartData(
              date: date,
              energy: data.energy ?? 0.0,
              cost: data.cost ?? 0.0,
            );
