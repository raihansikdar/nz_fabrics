...
class YearlyWaterGeneratorListWidget extends StatefulWidget {
  final Size size;

  const YearlyWaterGeneratorListWidget({super.key, required this.size});

  @override
  State<YearlyWaterGeneratorListWidget> createState() => _MonthlyDieselGeneratorListWidgetState();
}

class _MonthlyDieselGeneratorListWidgetState extends State<YearlyWaterGeneratorListWidget> {

  int onlySelectedYear = DateTime.now().year;
  @override
  void initState() {
    Get.find <YearlyWaterButtonController>().fetchYearlyWaterData(selectedYearDate: onlySelectedYear);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<YearlyWaterButtonController>(
        builder: (controller) {
          if(controller.isYearlyWaterDataInProgress){
            return  Padding(
          ...
          if(controller.yearlyWaterDataList.isEmpty){
            return  Padding(
              padding:  EdgeInsets.only(top: widget.size.height * 0.20),
          ...
          return Expanded(
            child: ListView.separated(
              ...
              shrinkWrap: true,
              primary: false,
              itemCount: controller.yearlyWaterDataList.length,
              itemBuilder: (context, index) {
                return CustomContainer(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        TextComponent(text: controller.yearlyWaterDataList[index].node ?? ''),
                        SizedBox(height: widget.size.height * k16TextSize),
                        _buildDataRow(size: widget.size, label: "Pressure", value: "${controller.yearlyWaterDataList[index].instantFlow?.toStringAsFixed(2) ?? '0.0'} L", textColor: const Color(0xFF793BE8), backgroundColor: const Color(0xFFf2ecfd)),
                        ...
                        _buildDataRow(size: widget.size, label: "Cost", value: "${controller.yearlyWaterDataList[index].cost?.toStringAsFixed(2) ?? '0.0'} BDT", textColor: const Color(0xFF7dcece), backgroundColor: const Color(0xFFe8f7f7)),
                        SizedBox(height: widget.size.height * k10TextSize),
                        _buildDataRow(size: widget.size, label: "Run Time", value: formatRuntime(controller.yearlyWaterDataList[index].runtime.toString()), textColor: const Color(0xFFa67def), backgroundColor: const Color(0xFFf2ecfd)),
                      ],
                    ),
                  ),
                );
