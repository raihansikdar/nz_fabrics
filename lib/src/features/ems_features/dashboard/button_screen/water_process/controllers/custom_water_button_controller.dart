...
import 'package:nz_fabrics/src/features/ems_features/dashboard/button_screen/water_process/model/custom_water_data_model.dart';
...
class CustomWaterButtonController extends GetxController with InternetConnectivityCheckMixin{

  bool _isConnected = true;
  bool _isCustomWaterButtonInProgress = false;
  String _errorMessage = '';
  List<CustomWaterDataModel> _customWaterDataList = <CustomWaterDataModel>[];

  bool get isConnected => _isConnected;
  bool get isCustomWaterButtonInProgress => _isCustomWaterButtonInProgress;
  String get errorMessage => _errorMessage;
  List<CustomWaterDataModel> get customWaterDataList => _customWaterDataList;
  ...
  Future<bool> fetchCustomWaterData({required String fromDate, required String toDate}) async {

    _isCustomWaterButtonInProgress = true;
    update();
    ...
    try {
      ...
      NetworkResponse response = await NetworkCaller.postRequest(
        url: Urls.postFilterWaterUrl,
        body: requestBody,
      );

      log("postFilterWaterUrl statusCode ==> ${Urls.postFilterWaterUrl}");

      log("postFilterWaterUrl statusCode ==> ${response.statusCode}");
      log("postFilterWaterUrl body ==> ${response.body}");

      _isCustomWaterButtonInProgress = false;
      update();

      if (response.isSuccess) {

        final jsonData = (response.body as Map<String, dynamic>)['data'] as List<dynamic>;
        _customWaterDataList = jsonData.map((json) => CustomWaterDataModel.fromJson(json as Map<String, dynamic>)).toList();

        update();
        return true;

      } else {
        _errorMessage = " Failed to fetch custom water data";
        update();
        return false;
      }
    } catch (e) {
      ...
      log('Error in fetching gas custom water data: $_errorMessage');
