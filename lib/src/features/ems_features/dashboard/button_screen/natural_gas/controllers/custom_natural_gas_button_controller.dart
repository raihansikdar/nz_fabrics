...
import 'package:nz_fabrics/src/features/ems_features/dashboard/button_screen/natural_gas/model/custom_natural_gas_data_model.dart';
...
class CustomNaturalGasButtonController extends GetxController with InternetConnectivityCheckMixin{

  bool _isConnected = true;
  bool _isCustomNaturalGasButtonInProgress = false;
  String _errorMessage = '';
  List<CustomNaturalGasDataModel> _customNaturalGasDataList = <CustomNaturalGasDataModel>[];

  bool get isConnected => _isConnected;
  bool get isCustomNaturalGasButtonInProgress => _isCustomNaturalGasButtonInProgress;
  String get errorMessage => _errorMessage;
  List<CustomNaturalGasDataModel> get customNaturalGasDataList => _customNaturalGasDataList;
  ...
  Future<bool> fetchCustomNaturalGasData({required String fromDate, required String toDate}) async {

    _isCustomNaturalGasButtonInProgress = true;
    update();
    ...
    try {
      ...
      NetworkResponse response = await NetworkCaller.postRequest(
        url: Urls.postFilterNaturalGasUrl,
        body: requestBody,
      );

      log("postFilterGasGeneratorUrl statusCode ==> ${Urls.postFilterNaturalGasUrl}");

      log("postFilterGasGeneratorUrl statusCode ==> ${response.statusCode}");
      log("postFilterGasGeneratorUrl body ==> ${response.body}");

      _isCustomNaturalGasButtonInProgress = false;
      update();

      if (response.isSuccess) {

        final jsonData = (response.body as Map<String, dynamic>)['data'] as List<dynamic>;
        _customNaturalGasDataList = jsonData.map((json) => CustomNaturalGasDataModel.fromJson(json as Map<String, dynamic>)).toList();

        update();
        return true;

      } else {
        _errorMessage = " Failed to fetch custom gas data";
        update();
        return false;
      }
    } catch (e) {
      ...
      log('Error in fetching gas custom day data: $_errorMessage');
