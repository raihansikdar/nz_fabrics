...
class TotalWaterSourceElementScreen extends StatefulWidget {
  final String categoryName;
  const TotalWaterSourceElementScreen({super.key, required this.categoryName});

  @override
  State<TotalWaterSourceElementScreen> createState() => _TotalWaterSourceElementScreenState();
}

class _TotalWaterSourceElementScreenState extends State<TotalWaterSourceElementScreen> {


  @override
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_){
      Get.find<WaterEachCategoryWiseLiveDataController>().fetchEachCategoryLiveData(categoryName: widget.categoryName);
      ...
      body:  GetBuilder<WaterEachCategoryWiseLiveDataController>(
          builder: (waterEachCategoryWiseLiveDataController) {
            return ListView.separated(
              ...
              itemCount: waterEachCategoryWiseLiveDataController.isLoading ? 8 :
              waterEachCategoryWiseLiveDataController.waterEachCategoryDataList.isEmpty ?
              1 :  waterEachCategoryWiseLiveDataController.hasError ? 1 :
              waterEachCategoryWiseLiveDataController.waterEachCategoryDataList.length,
              itemBuilder: (context,index){
                if(waterEachCategoryWiseLiveDataController.isLoading){
                  return Padding(
                    padding: EdgeInsets.all(size.height * k8TextSize),
                    child: CustomShimmerWidget(height: size.height * 0.09 , width: double.infinity,),
                  );
                } else if(waterEachCategoryWiseLiveDataController.waterEachCategoryDataList.isEmpty || waterEachCategoryWiseLiveDataController.hasError){
                  return Center(child: Lottie.asset(AssetsPath.emptyJson,height: size.height * 0.250));
                } else if(waterEachCategoryWiseLiveDataController.hasError){
                  return ErrorPage(onRetry: (){
                    waterEachCategoryWiseLiveDataController.fetchEachCategoryLiveData(categoryName: widget.categoryName);
                  });
                }

                else{
                  dynamic waterCategoryData = waterEachCategoryWiseLiveDataController.waterEachCategoryDataList[index];
