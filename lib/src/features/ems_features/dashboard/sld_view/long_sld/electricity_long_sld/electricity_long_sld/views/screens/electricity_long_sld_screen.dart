...
//     await prefs.setString('electricity_long_sld_viewPageData_time', DateTime.now().toIso8601String());
//     await prefs.setString(
...
// }
//
// class _ElectricityLongSldScreenState extends State<ElectricityLongSldScreen> with SingleTickerProviderStateMixin, WidgetsBindingObserver {
//   List<ElectricityLongViewPageModel> _viewPageData = [];
//   Map<dynamic, LiveDataModel> _liveData = {};
//   bool _isLoading = true;
//   late AnimationController _controller;
...
//     _fetchPFData();
//
//     // Fetch live node power and start periodic updates
//     final nodePowerController = Get.find<ElectricityLongSLDLiveAllNodePowerController>();
//     nodePowerController.fetchLiveAllNodePower();
//     nodePowerController.startApiCallOnScreenChange();
//
//     // Stop other controllers
//
//     Get.find<CategoryWiseLiveDataController>().stopApiCallOnScreenChange();
...
//     Get.find<ElectricityShortSLDLiveAllNodePowerController>().stopApiCallOnScreenChange();
//
//     ever(nodePowerController.liveAllNodePowerModel, (List<ElectricityLongLiveAllNodePowerModel> newData) {
//       _updateLiveData(newData);
//     });
//
//     WidgetsBinding.instance.addObserver(this);
//   }
...
//   void _updateLiveData(List<ElectricityLongLiveAllNodePowerModel> nodePowerData) {
//     _debounceTimer?.cancel();
//     _debounceTimer = Timer(const Duration(milliseconds: 500), () {
//       final newLiveData = <dynamic, LiveDataModel>{};
//       newLiveData.addAll(_liveData);
//
//       // Update from API data
//       for (var node in nodePowerData) {
//         if (node.node != null) {
//           newLiveData[node.node!] = LiveDataModel(
//             power: node.power ?? 0.0,
//             sensorStatus: (node.power ?? 0.0) != 0.0,
//           );
//         }
//       }
//
//       // Ensure all nodes in _viewPageData have entries
//       for (var item in _viewPageData) {
//         newLiveData.putIfAbsent(
//           item.nodeName,
//               () => LiveDataModel(power: 0.0, sensorStatus: false),
//         );
//       }
//
//       if (!_areLiveDataEqual(_liveData, newLiveData)) {
//         setState(() {
...
//     });
//   }
//   bool _areLiveDataEqual(Map<dynamic, LiveDataModel> oldData, Map<dynamic, LiveDataModel> newData) {
//     if (oldData.length != newData.length) return false;
//     for (var key in oldData.keys) {
//       if (!newData.containsKey(key) ||
//           oldData[key]!.power != newData[key]!.power ||
//           oldData[key]!.sensorStatus != newData[key]!.sensorStatus) {
//         return false;
//       }
//     }
//     return true;
...
//
//         if (cachedLiveData != null) {
//           parsedLiveData = Map<dynamic, LiveDataModel>.from(json
//               .decode(cachedLiveData)
//               .map((id, data) => MapEntry(id, LiveDataModel.fromJson(data))));
//         }
//
//         // Initialize _liveData with defaults for all nodes in parsedData
//         for (var item in parsedData) {
//           parsedLiveData.putIfAbsent(
//             item.nodeName,
//                 () => LiveDataModel(power: 0.0, sensorStatus: false),
//           );
//         }
//
//         // Set state and ensure animation is running
//         setState(() {
//           _viewPageData = parsedData;
//           _liveData = parsedLiveData;
//           _isLoading = false;
...
//     _isFetchingViewPageData = true;
//     final requestId = Uuid().v4();
//     debugPrint('[$requestId] API Call : ViewPageData at ${DateTime.now()}');    try {
//       if (_viewPageData.isEmpty) {
//         setState(() {
//           _isLoading = true;
//         });
...
//       Future.delayed(const Duration(seconds: 2), () {
//         startTimer();
//         // Ensure node power controller is active
//         final nodePowerController = Get.find<ElectricityLongSLDLiveAllNodePowerController>();
//         nodePowerController.startApiCallOnScreenChange();
//         nodePowerController.fetchLiveAllNodePower();
//         // Restart animation
...
//                             children: [
//                               CustomPaint(
//                                 key: const ValueKey('sld_painter'),
//                                 size: Size(contentWidth, contentHeight),
//                                 painter: ElectricityLongSLDAnimatedLinePainter(
//                                   viewPageData: _viewPageData,
//                                   liveData: _liveData,
...
//                                   minY: minY,
//                                   animation: _controller.view,
//                                   controller: Get.find<ElectricityLongSLDGetAllInfoUIControllers>(),
//                                 ),
//                               ),
//                               ..._buildWidgets(minX, minY),
//                               ..._buildPFWidgets(minX, minY),
...
//                     loadBoxWidth: item.width.toDouble(),
//                     onTap: () {
//                       Get.to(() => ElectricityLongSLDMainBusBarTrueScreen(
//                           busBarName: item.nodeName,
//                         ),
//                         transition: Transition.rightToLeft,
//                         duration: const Duration(seconds: 1),
