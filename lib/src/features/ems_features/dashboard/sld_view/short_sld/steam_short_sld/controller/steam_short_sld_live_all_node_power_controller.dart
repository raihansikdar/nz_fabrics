...
class SteamShortSLDLiveAllNodePowerController extends GetxController with InternetConnectivityCheckMixin,WidgetsBindingObserver {


  bool _isConnected = true;
  bool _isLiveAllNodePowerInProgress = false;
  String _errorMessage = '';
  List<SteamShortLiveAllNodePowerModel>  _liveAllNodePowerModel = [];

  bool get isConnected => _isConnected;
  bool get isLiveAllNodePowerInProgress => _isLiveAllNodePowerInProgress;
  String get errorMessage => _errorMessage;
  List<SteamShortLiveAllNodePowerModel>  get liveAllNodePowerModel => _liveAllNodePowerModel;
  ...
  void stopApiCallOnScreenChange() {
    if (Get.isRegistered<SteamShortSLDLiveAllNodePowerController>()) {
      final controller = Get.find<SteamShortSLDLiveAllNodePowerController>();
      controller._stopPeriodicApiCall();
      log("SteamShortSLDLiveAllNodePowerController Stop Api Call");
    }
  }

  void startApiCallOnScreenChange() {
    if (!Get.isRegistered<SteamShortSLDLiveAllNodePowerController>()) {
      final controller = Get.put(SteamShortSLDLiveAllNodePowerController());
      controller._startPeriodicApiCall();
    } else {
      final controller = Get.find<SteamShortSLDLiveAllNodePowerController>();
      controller._startPeriodicApiCall();
      log("SteamShortSLDLiveAllNodePowerController Start Api Call");
    }
    _isStopApiCall = false;
    update();
    ...
    try {
     ...
     // log("getLiveAllNodePowerUrl body ==> ${response.body}");
      ...
      if (response.isSuccess) {
        ...
        _liveAllNodePowerModel = jsonData.map((json)=> SteamShortLiveAllNodePowerModel.fromJson(json)).toList();
