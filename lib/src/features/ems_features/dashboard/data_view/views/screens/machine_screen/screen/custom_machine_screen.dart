...
//
//
// class CustomMachineScreen extends StatefulWidget {
//   const CustomMachineScreen({super.key});
//
//   @override
//   State<CustomMachineScreen> createState() => _CustomMachineScreenState();
// }
//
// class _CustomMachineScreenState extends State<CustomMachineScreen> {
//   late List<DataGridRow> _dataGridRows = [];
//   late List<DataGridRow> _filteredRows = [];
//   late Map<String, dynamic> _apiData = {};
//   late Map<String, bool> _expandState = {};
//
...
//     }
//
//     final apiUrl1 = '${Urls.baseUrl}/api/filter-machine-view-data/';
...
//           // Parent nodes should always be acknowledged
//           if (value['children'] != null && (value['children'] as List).isNotEmpty) {
//             value['acknowledged'] = true;
//           } else if (apiData2.containsKey(key)) {
//             // For non-parent nodes, use child acknowledgment logic
...
//     // If no children, assume acknowledged (true)
...
//   }
// // Modify _buildDataGridRows to handle acknowledgment hierarchy
...
//                     ),
//                     child: Center(child: TextComponent(text: 'Submit',color: AppColors.whiteTextColor,fontSize:  size.width > 500 ? size.width * 0.025 : size.width * 0.035,)),
//
//                   ),
//                 )
//               ],
...
//               color: Colors.white,
//             ),
//             child: _isLoading ?  Center(child: Lottie.asset(AssetsPath.loadingJson, height: size.height * 0.12)) : SfDataGridTheme(
...
//                       child: const Text(
//                         'Node',
//                         style: TextStyle(fontWeight: FontWeight.bold,color: AppColors.whiteTextColor,fontSize: 16),
//                       ),
//                     ),
//                   ),
//                   GridColumn(
...
//     required this.context,  // Accept context
...
//             columnName: 'Acknowledged', value: value['acknowledged'] ?? false),
//         DataGridCell<double>(
//             columnName: 'Energy', value: (value['total_energy'] as num?)?.toDouble() ?? 0.0),
//         DataGridCell<double>(
//             columnName: 'Cost', value: (value['total_cost'] as num?)?.toDouble() ?? 0.0),
//       ]));
//
...
//             DataGridCell<bool>(
//                 columnName: 'Acknowledged', value: child['acknowledged'] ?? false),
//             DataGridCell<double>(
//                 columnName: 'Energy', value: (child['total_energy'] as num?)?.toDouble() ?? 0.0),
//             DataGridCell<double>(
//                 columnName: 'Cost', value: (child['total_cost'] as num?)?.toDouble() ?? 0.0),
//           ]));
//         }
//       }
//     });
...
//     final isParentNode = !node.contains('•');  // Parent node doesn't have a bullet point
...
//
//     // Parent and child icons (conditional)
//     String iconPath = isParentNode
//         ? (isAcknowledged
//         ? AssetsPath.imgIconPNG  // Acknowledged parent
//         : AssetsPath.imgIconPNG)  // Non-acknowledged parent
//         : (isAcknowledged
//         ?  AssetsPath.twitch1IconPNG  // Acknowledged child
//         : AssetsPath.twitchIconPNG);  // Non-acknowledged child
//
...
//             Get.find<EachMachineWiseLoadLiveDataController>().fetchEachCategoryLiveData(categoryName: cleanNodeName);
//           } if (!isParentNode) {
//             String cleanNodeName = node.replaceAll('•', '').trim();
//             log(cleanNodeName);
//             Navigator.push(
...
//           if (!isParentNode) {
//             String cleanNodeName = node.replaceAll('•', '').trim();
//             log(cleanNodeName);  // Log the cleaned node name for verification
//             Get.to(
//                   () => AcKnowledgeHistoryScreen(nodeName: cleanNodeName),
...
//         padding: const EdgeInsets.all(8.0),
//         child: Text(
//           _formatDouble(row.getCells()[2].value, unit: 'kWh'),
//         ),
//       ),
//       Container(
//         alignment: Alignment.center,
//         padding: const EdgeInsets.all(8.0),
//         child: Text(
//           _formatDouble(row.getCells()[3].value, unit: '৳'),
//         ),
//       ),
//     ]);
//
...
//
//
//   String _formatDouble(dynamic value, {String? unit}) {
...
//   }
//
//   // Table 1: CustomMachineScreen data fetch
...
//         });
//       } else {
//         throw Exception('Failed to fetch Table 1 data');
//       }
//     } catch (e) {
//       log('Table 1 Error: $e');
//     } finally {
...
//   }
//
//   // Table 2: MachineDataPage data fetch
...
//           });
//         } else {
//           throw Exception('Empty response for Table 2');
//         }
//       } else {
...
//         if (machineNames.isEmpty) machineNames = ['No machines available'];
//       });
//     } finally {
//       setState(() {
...
//
//         // Sheet 1: Day Wise Machine Report (Table 2)
...
//
//         // Set headers for Machine Report
...
//           machineSheet.getRangeByIndex(1, colIndex + 1).setText('$machine Cost (৳)');
//           colIndex += 2;
//         }
//
//         // Apply header style
...
//             machineSheet.getRangeByIndex(rowIndex, colIndex + 1).setNumber(machines[machine]?['cost'] ?? 0);
//             colIndex += 2;
//           }
//           rowIndex++;
//         });
...
//         colIndex = 2;
//         for (var machine in machineNames) {
//           machineSheet.getRangeByIndex(rowIndex, colIndex).setNumber(totalEnergyMap[machine] ?? 0);
//           machineSheet.getRangeByIndex(rowIndex, colIndex + 1).setNumber(totalCostMap[machine] ?? 0);
//           colIndex += 2;
//         }
//
//         // Sheet 2: Custom Machine Report (Table 1)
//         final Worksheet nodeSheet = workbook.worksheets.add();
//         nodeSheet.name = 'Custom Machine Report';
//
//         // Set headers for Node Report
...
//                     height: size.width > 500 ? size.height * 0.4 : size.height * 0.5,
//
//                     margin: const EdgeInsets.only(left: 8.0,right: 8.0,bottom: 50,top: 8),
//                     clipBehavior: Clip.antiAlias,
//                     decoration: BoxDecoration(
...
//                         if (snapshot.connectionState == ConnectionState.waiting && _isLoadingTable2) {
...
// }
//
// // Reuse CustomTreeDataSource and MachineDataGridSource as provided
// class CustomTreeDataSource extends DataGridSource {
//   CustomTreeDataSource({
//     required List<DataGridRow> dataGridRows,
//     required this.apiData,
...
//     );
//
//     // Calculate heights for DataGrids
...
//             // Table 2: MachineDataPage
//             Container(
//
//               margin: const EdgeInsets.only(left: 8.0, right: 8.0, bottom: 50, top: 0),
//               clipBehavior: Clip.antiAlias,
//               decoration: BoxDecoration(
//                 borderRadius: BorderRadius.circular(12),
//                 color: Colors.white,
...
//   }
//
//   List<GridColumn> _buildTable1Columns() {
//     return [
//       GridColumn(
//         columnName: 'Node',
//         label: Container(
